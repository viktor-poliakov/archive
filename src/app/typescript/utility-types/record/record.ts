...
// С литеральными ключами такой ловушки не бывает — все ключи на месте.`;
  ...
  protected readonly ownImplementation = `// Record не встроен в компилятор — он написан обычным кодом
// в стандартной библиотеке TypeScript (lib.es5.d.ts). Целиком он такой:

// ── Record: «запись» (строка картотеки «ключ → значение») ────────
// «Для каждого имени P из набора K создай поле P со значением T».
type MyRecord<K extends keyof any, T> = {
  [P in K]: T;
  //        └─ просто T, БЕЗ зависимости от P:
  //           у всех ключей значение одного типа
};
// keyof any — это string | number | symbol, то есть «всё, чем может
// быть имя свойства». Так Record разрешает любой годный тип ключа.

// Сравните с Pick — там на том же месте стоит T[P]:
//   Pick:   [P in K]: T[P]   ← у каждого ключа СВОЙ тип
//   Record: [P in K]: T      ← у всех ключей ОДИН тип
// Одна буква разницы, а смысл противоположный.

// ── Режим 1: K — объединение литералов ──────────────────────────
type Role = 'admin' | 'editor' | 'guest';

type Permissions = MyRecord<Role, boolean>;
// Цикл пробежал по трём именам и развернулся в обычный объект:
// {
//   admin: boolean;
//   editor: boolean;
//   guest: boolean;
// }
// Все три поля ОБЯЗАТЕЛЬНЫ — вот откуда «заполни все ключи».

const perms: Permissions = {
  admin: true,
  editor: true,
  guest: false,
}; // ✅ то же самое, что встроенный Record<Role, boolean>

// ── Режим 2: K — string ─────────────────────────────────────────
type Counter = MyRecord<string, number>;
// Пробежать по бесконечному множеству строк нельзя, поэтому
// компилятор сворачивает такой цикл в ИНДЕКСНУЮ СИГНАТУРУ:
// { [key: string]: number }
//
// Это не «почти то же самое», а буквально она —
// ровно поэтому Record<string, V> и { [key: string]: V } равны.`;
}
