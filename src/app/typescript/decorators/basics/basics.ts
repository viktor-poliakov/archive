...
// Но @-значок сам по себе аргументы не принимает. Решение — обычная
...
@logWithPrefix('СОЗДАН КЛАСС:')   // ← со скобками! Сначала вызываем функцию...
...
@logWithPrefix()   // СО скобками: сначала вызываем функцию, она вернёт декоратор.
class B {}

// Если перепутать — будет ошибка или странное поведение:
// @logged()  — попытка ВЫЗВАТЬ logged, но он не возвращает декоратор → ошибка.
// @logWithPrefix — повесить саму ФУНКЦИЮ как декоратор, а не её результат → тоже сломается.

// Правило: @имя — готовый декоратор; @имя(...) — функция, которую надо вызвать.`;

  protected readonly standardVsLegacy = `// В TypeScript уживаются ДВА поколения декораторов. Важно их различать.

// 1) СТАНДАРТНЫЕ (современные) — те, что описаны в ОБЩЕМ предложении для
//    JavaScript (TC39, стадия 3). В стандарт языка они ещё не вошли, и ни
//    браузер, ни Node их пока не исполняют. Но TypeScript 5.0+ понимает их
//    без всяких настроек и превращает в обычный код при компиляции.
//    Мы учим именно их. Сигнатура: (value, context).
